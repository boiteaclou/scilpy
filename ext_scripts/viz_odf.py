...
fodf_f = "/mnt/i/scil_master/experiments/check_bval/outputs/sub0_rep0/fodf/sub0_rep0_fodf.nii.gz"
fodf_img = nib.load(fodf_f)
...
def click_x_label(i_ren, *_):
    global x_slicer_on
    if x_slicer_on:
        x_slicer_on = False
        odf_actor_x.VisibilityOff()
    else:
        x_slicer_on = True
        odf_actor_x.VisibilityOn()

    i_ren.force_render()


def click_y_label(i_ren, *_):
    global y_slicer_on
    if y_slicer_on:
        y_slicer_on = False
        odf_actor_y.VisibilityOff()
    else:
        y_slicer_on = True
        odf_actor_y.VisibilityOn()

    i_ren.force_render()


def click_z_label(i_ren, *_):
    global z_slicer_on
    if z_slicer_on:
        z_slicer_on = False
        odf_actor_z.VisibilityOff()
    else:
        z_slicer_on = True
        odf_actor_z.VisibilityOn()
...
line_slider_label_x.on_left_mouse_button_clicked = click_x_label
line_slider_label_y.on_left_mouse_button_clicked = click_y_label
line_slider_label_z.on_left_mouse_button_clicked = click_z_label

save_button = ui.Button2D((("save", read_viz_icons(fname="floppy-disk.png")),))
save_name = ui.TextBox2D(35, 20, "filename")
...
panel.add_element(save_button, (0.1, 0.15))
panel.add_element(save_name, (0.4, 0.15))

show_m.scene.add(panel)


def save_image(i_ren, *_):
    combobox.set_visibility(False)
    panel.set_visibility(False)
    i_ren.force_render()
    window.record(scene, out_path="{}.png".format(save_name.text),
...
save_button.on_left_mouse_button_clicked = save_image
